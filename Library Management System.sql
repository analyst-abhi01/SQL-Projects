...
#Task-2. Update an Existing Member's Address where C=103 from 798 Oak St to 125 Oak St.
UPDATE members
...
SELECT
    issued_emp_id, COUNT(*) as books_cnt
FROM issued_status
...
SELECT b.isbn, b.book_title, COUNT(ist.issued_id) AS issued_cnt
...
# Task16: CTAS: Create a Table of Active Members
...
# Write a query to find the top 3 employees who have processed the most book issues. Display the employee name, number of books processed, and 
# their branch.
SELECT 
    e.emp_name,
    b.*,
...
SELECT 
    ...
    COUNT(CASE WHEN DATEDIFF(CURDATE(), i.issued_date) > 30 AND i.return_date IS NULL THEN 1 END) AS overdue_books_count,
    SUM(CASE WHEN DATEDIFF(CURDATE(), i.issued_date) > 30 AND i.return_date IS NULL 
             THEN (DATEDIFF(CURDATE(), i.issued_date) - 30) * 0.50 ELSE 0 END) AS total_fines,
    COUNT(i.issued_book_isbn) AS total_books_issued
FROM 
    issued_status i
GROUP BY 
    i.issued_member_id;
